...
# Note: used only in dotnet build
# Not sure if we need app bundle or simply direct
...
#    # This exists in /src/App.CLI.MacOS.Elevated/build.sh and in /src/eddie.macos.postbuild.sh
#    ARCH=arm64
#fi
#if [ "$ARCH" == "x86_64" ]; then
#    ARCH=x64
...
if [ $CONFIG = "Release" ]; then
    ...
    ${BASEPATH}/../repository/macos_common/sign.sh "$OUTPATH/eddie-cli-elevated" no yes
fi
...
if [ $CONFIG = "Release" ]; then
    ...
    ${BASEPATH}/../repository/macos_common/sign.sh "$OUTPATH/libLib.Platform.MacOS.Native.dylib" no yes
fi
