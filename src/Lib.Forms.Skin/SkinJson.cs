...
namespace Eddie.Forms.Skin
{
	public class JsonValue
	{
		...
		public JsonValue(SkinJson json, string key, int? index)
		{
			m_json = json;
			m_key = key;
			m_index = index;
		...
		public SkinJson Json
		{
			get
			{
				return Value as SkinJson;
			}
		}
		...
		public string ValueString
		{
			get
			{
				return SkinUtilsCore.ToString(Value);
			}
		}
		...
		public bool ValueBool
		{
			get
			{
				return SkinUtilsCore.ToBool(Value);
			}
		}
		...
		public int ValueInt
		{
			get
			{
				return SkinUtilsCore.ToInt32(ValueString);
			}
		}
		...
		public Int64 ValueInt64
		{
			get
			{
				return SkinUtilsCore.ToInt64(Value);
			}
		}
	...
	public class SkinJson : DynamicObject
	{
		...
		public static SkinJson Parse(string s)
		{
			SkinJson j = new SkinJson();
			j.FromJson(s);
			return j;
		}

		public static bool TryParse(string s, out SkinJson j)
		{
			object v;
			string r;
		...
		public void FromJson(string s)
		{
			...
			if (result != null)
			{
				if (result is SkinJson)
					m_value = (result as SkinJson).m_value;
				else
					m_value = result;
			}
			else
		...
		public SkinJson Clone() // TOOPTIMIZE
		{
			return SkinJson.Parse(this.ToJson());
		}
		...
		{
			if (m_value is SkinJson)
			{
				(m_value as SkinJson).ToJson(sb, indent, indentN);
			}
			else if (IsDictionary())
			{
				...
				foreach (JsonDictionaryKeyValuePair kp in valueDictionary)
				{
					...
					if (indent)
						sb.Append(' ');
					new SkinJson(kp.Value).ToJson(sb, indent, indentN + 1);
			...
			else if (IsArray())
			{
				...
				foreach (object v in valueArray)
				{
					...
					if (indent)
						sb.Append('\t', indentN);
					new SkinJson(v).ToJson(sb, indent, indentN + 1);
