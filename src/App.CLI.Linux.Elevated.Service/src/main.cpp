...
void cleanService()
{
	...
	if (access(systemdUnitPath.c_str(), F_OK) == 0)
	{	
		execCommand((const char *const[]){"/usr/bin/systemctl", "stop", "eddie-elevated.service", NULL});
		execCommand((const char *const[]){"/usr/bin/systemctl", "disable", "eddie-elevated.service", NULL});
		remove(systemdUnitPath.c_str());
	}
