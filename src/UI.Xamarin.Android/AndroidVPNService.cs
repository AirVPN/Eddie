...
{
	[Service(Name="com.eddie.android.VPNService", Permission="android.permission.BIND_VPN_SERVICE")]
	[IntentFilter(new[] { global::Android.Net.VpnService.ServiceInterface })]
	public class AndroidVPNService : global::Android.Net.VpnService, IMessageHandler
	{
		...
		public override StartCommandResult OnStartCommand(Intent intent, StartCommandFlags flags, int startId)
		{
			...
			if(intent != null)
			{
				...
				{
					LogsManager.Instance.Debug("AndroidVPNService.OnStartCommand: START");
				
					run = true;
				}					
				else if(intent.GetBooleanExtra(PARAM_STOP, false))
				{
					LogsManager.Instance.Debug("AndroidVPNService.OnStartCommand: STOP");
			...
			{
				LogsManager.Instance.Debug("AndroidVPNService.OnStartCommand (no intent)");
			}
		...
		{
			LogsManager.Instance.Error("AndroidVPNService.OnRevoke");
		...
		public void OnMessage(Message msg)
		{
			...
			switch(msg.What)
			{
			case AndroidVPNService.MSG_BIND:	UpdateClients(msg.ReplyTo, msg.Arg1 == MSG_BIND_ARG_ADD ? true : false);
												break;

			case AndroidVPNService.MSG_START:	UpdateService(true, msg.Data);
												break;

			case AndroidVPNService.MSG_STOP:	UpdateService(false);
												break;
			}

			if(msg.ReplyTo != null)
			{
				VPN.ServiceStatus status;
				string lastError;
		...
		private bool UpdateService(bool run, Bundle data = null)
		{
			lock(m_dataSync)
			{
				bool running = (m_status == VPN.ServiceStatus.Started) || (m_status == VPN.ServiceStatus.Starting);
				if(running == run)
					return true;

				if(m_status == VPN.ServiceStatus.Stopping)
		...
		{
			OptionsManager.SystemLastProfileActivated = m_lastProfile;
		...
		{
			LogsManager.Instance.Debug(string.Format("AndroidVPNService.OnScreenChanged: active='{0}'", active));
		...
		{
			DoChangeStatus(VPN.ServiceStatus.Started);
		...
		private void DoStopService()
		{
			...
			DoStopForeground();
			StopSelf();
			DoChangeStatus(VPN.ServiceStatus.Stopped);
			OnServiceStopped();
		}
		...
		{
			LastError = "";
			DoChangeStatus(VPN.ServiceStatus.Starting);
		...
		private void ResetLastProfile()
		{
			m_lastProfile = "";
			OptionsManager.SystemLastProfileActivated = "";
		}
		...
		private void DoStop()
		{
			...
			DoChangeStatus(VPN.ServiceStatus.Stopping);
		...
		{
			Intent configIntent = new Intent(this, typeof(OptionsActivity));
		...
		{
			if(OptionsManager.SystemShowNotification && (m_notification == null))
			{
				m_notification = new Notification.Builder(this)
							.SetContentTitle("Eddie")
							.SetContentText("Eddie is running")
