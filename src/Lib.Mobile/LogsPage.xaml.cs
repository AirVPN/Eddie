...
namespace Eddie
{
	...
	{
		private const int MAX_LOGS = 250;

		private bool m_hasLogHandler = false;
		...
		private ObservableCollection<LogItem> m_logsCollection = new ObservableCollection<LogItem>();
		...
		private void OnLog(string content, bool bind)
		{
			m_logsCollection.Add(new LogItem { Content = content });
						
			while(m_logsCollection.Count > MAX_LOGS)
			{
				m_logsCollection.RemoveAt(0);
			}
		...
		private void InitUI()
		{
			m_logsView.ItemsSource = m_logsCollection;

			m_buttonShare.Clicked += OnShareLog;			
		}
...
.w3-mobile .w3-dropdown-content{position:relative}	
...
.w3-circle{border-radius:50%}.w3-round-small{border-radius:2px}.w3-round,.w3-round-medium{border-radius:4px}
.w3-round-large{border-radius:8px}.w3-round-xlarge{border-radius:16px}.w3-round-xxlarge{border-radius:32px}
...
<body>
	<div class='container'>
		<div class='page'>";
			...
			foreach(LogItem log in m_logsCollection)
			{
				html += "<div>" + log.Content + "</div><br/>";
			}
		...
		private string GetLogContent()
		{
			...
			foreach(LogItem item in m_logsCollection)
			{
				if(content.Length > 0)
					content += "\r\n";
		...
		private void OnShareLog(object sender, System.EventArgs e)
		{
			string content = GetLogContent().Trim();
			if(Utils.Empty(content))
				return;

			App.Instance.VPNManager.ShareContent("Log", content);
		}
		...
		protected override void OnAppearing()
		{
			...
			UpdateLogHandler(true);
		}

		protected override void OnDisappearing()
		{
			UpdateLogHandler(false);
		...
		private void OnLogEvent(LogEntry e)
		{
			...
			Device.BeginInvokeOnMainThread(() => HandleLogEvent(message));			
		}
	}
}
