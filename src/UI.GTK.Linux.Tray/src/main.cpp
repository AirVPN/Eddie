...
//static GtkWidget *g_icon_normal = nullptr;
//static GtkWidget *g_icon_gray = nullptr;
...
static void clearImages()
{
    ...
    g_imagesMap.clear();
    /*
    if(g_icon_normal != nullptr)
    {
        gtk_image_clear(GTK_IMAGE(g_icon_normal));
        g_icon_normal = nullptr;
    }

    if(g_icon_gray != nullptr)
    {
        gtk_image_clear(GTK_IMAGE(g_icon_gray));
        g_icon_gray = nullptr;
    }
    */
}
...
{
    //std::cerr << "Usage: " << argv[0] << " [-r] [input pipe file] [-w] [output pipe file] [-p] [root path]" << std::endl;
    std::cerr << "Do not execute this app directly , it's used by Eddie" << std::endl;
}
/*
static bool parseCommand(const std::string &str, std::string &cmd, std::vector<std::string> &args)
{
...
/*
static GtkWidget * loadEmbeddedImage(const guint8 *data)
{
    GdkPixbuf *pixbuf = gdk_pixbuf_new_from_inline(-1, data, FALSE, nullptr);
    if(!pixbuf)
        return nullptr;
...
?{
    g_icon_normal = loadEmbeddedImage(icon_normal_inline);
    g_icon_gray = loadEmbeddedImage(icon_gray_inline);

    return g_icon_normal && g_icon_gray;
}
*/
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
