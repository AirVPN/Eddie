...
namespace Eddie.Forms.Forms
{
	public partial class WindowProviderEditWireGuard : Eddie.Forms.Skin.SkinForm
	{
		...
		{
			base.OnLoad(e);
			CommonInit(LanguageManager.GetText(LanguageItems.WindowsProviderEditWireGuardTitle));
			...
			chkSupportIPv6.Text = LanguageManager.GetText(LanguageItems.WindowsProviderEditWireGuardSupportIPv6);
		...
		{
			string result = GuiUtils.DirectoryPicker(LanguageManager.GetText(LanguageItems.WindowsProviderEditWireGuardPathBrowse), txtPath.Text);
			if (result != "")
				txtPath.Text = result;
		}
