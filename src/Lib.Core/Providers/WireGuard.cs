...
{
	public class WireGuard : IProvider
	{
		...
		{
			ConnectionTypes.IConnectionType c = new ConnectionTypes.WireGuard();
			return c;
		}
		...
		public string Path
		{
			get
			{
				...
				if (path == "")
					path = Platform.Instance.GetDefaultWireGuardConfigsPath();
				return path;
			}
			set
			{
			...
			try
			{
				foreach (string filePath in Directory.GetFiles(path))
				{
					...
					try
					{
						...
						ConfigBuilder.WireGuard parser = new ConfigBuilder.WireGuard();
						parser.Parse(config);

						string hosts = parser.PeerEndpointAddress.ToString();
		...
		public string ComputeFriendlyNameFromPath(string path)
		{
			...
			name = Regex.Replace(name, ".conf", "", RegexOptions.IgnoreCase);
