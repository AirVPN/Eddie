	...
	public class App
	{
		public static void OpenStats(string key)
		{
			if (key == "vpngeneratedconfig")
			{
				...
				{
					Engine.Instance.OnShowText(LanguageManager.GetText(LanguageItems.StatsVpnGeneratedConfig), Engine.Instance.Connection.ExportConfigStartup());
				}
			}
			else if (key == "vpngeneratedconfigpush")
			{
				if (Engine.Instance.IsConnected())
				{
					Engine.Instance.OnShowText(LanguageManager.GetText(LanguageItems.StatsVpnGeneratedConfigPush), Engine.Instance.Connection.ExportConfigPush());
				}
			}
			else if (key == "pinger")
			{
