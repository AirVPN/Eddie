...
{
	public class OpenVPN : IConfigBuilder
	{
		...
		public OpenVPN Clone()
		{
			// Note: only directives
			OpenVPN n = new OpenVPN();
		...
		{
			if (ExistsDirective("remote") == false)
				return "Missing 'remote' directive";
			return "";
		}
		...
		public override string Build()
		{
			...
			result += "# " + Utils.GenerateFileHeader() + Platform.Instance.EndOfLineSep + Platform.Instance.EndOfLineSep;
		...
		public override void Parse(string config)
		{
			...
			AppendDirectives(config, "Original");
		}
		...
		public void RemoveDirectiveValueStart(string name, string value)
		{
			if (Directives.ContainsKey(name) == false)
				return;
		...
		// Virtual
		public virtual bool GetSupportCommentInConfig()
		{
			return true;
		}
	}
